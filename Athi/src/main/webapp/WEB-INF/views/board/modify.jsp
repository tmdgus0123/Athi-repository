	...
	<div class="container">
		...
		<div class="col-sm-12" style="margin-top: 50px; margin-bottom: 50px;">
			<div class="col-sm-12 text-center" style="background-color: white; border-radius: 1em; padding-top: 25px; margin-bottom: 25px; opacity: 0.85;">
				<form name="modifyFrm" action="pModifyAction.do" method="post" onsubmit="return checkWriteFrm();">
					...
					<input type="hidden" name="id" value="${postInfo.id}" readonly/>
					...
					<div class="row form-group" style="background-color: #e7adb7; height: 50px">
						<div class="col-sm-11 text-right" style="margin: auto;">글쓴이 : ${postInfo.id }</div>
					...
					<div class="row form-group">
						<c:choose>
							<c:when test="${param.boardName=='QnABoard' or param.boardName=='tipBoard'}">
								<div class="col-sm-12">
									<textarea name="content" class="form-control" style="height: 400px;">${postInfo.content}</textarea>
								</div>
							</c:when>
							<c:otherwise>
