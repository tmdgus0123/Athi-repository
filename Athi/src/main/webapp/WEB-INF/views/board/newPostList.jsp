...
<body style="background:white;">
	<div class="row col-sm-12">
		<h1 class="text-center">&lt; New 게시물 리스트 &gt;</h1>
	</div>
	<table class="table table-bordered table-striped table-responsive" id="dataTable" cellspacing="0">
		...
		<tbody>
			<!-- 반복문 시작 -->
			<c:choose>
				<c:when test="${empty newPostList }">
					<tr style="text-align: center;">
						<td colspan="5">등록된 글이 없습니다.</td>
					</tr>
				</c:when>
				<c:otherwise>
					<c:forEach items='${newPostList }' var='list' varStatus='loop'>
						<tr>
							<td>${list.num}</td>
							<td>${list.title}</td>
							<td>
							...
							<td>								
								<c:choose>
									<c:when test="${list.p_language ge null }">
										없음.
									</c:when>
									<c:otherwise>
										${list.p_language}
